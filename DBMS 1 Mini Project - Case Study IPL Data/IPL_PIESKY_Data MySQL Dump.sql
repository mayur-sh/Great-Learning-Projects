...
select bidder_id, bidder_name , ds/d*100 as win_percentage 
from (select bidder_id, bid_status,count(bid_status) as ds 
from ipl_bidding_details where bid_status = "won" group by bidder_id) a2 join 
 (select bidder_id, bid_status,count(bid_status) as d from ipl_bidding_details group by bidder_id)
 a1 using(bidder_id) join ipl_bidder_details using (bidder_id) order by win_percentage desc;


-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.

select stadium_id, stadium_name, city , count(match_id) from ipl_match_schedule a join ipl_stadium b using(stadium_id) group by stadium_id;


-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?

select ds/dd*100 as "percentage of wins by a team which has won the toss", t.stadium_name from 
(select count(match_id) ds,stadium_id,stadium_name from ipl_match join ipl_match_schedule using(match_id) join ipl_stadium using(stadium_id) where toss_winner = match_winner 
group by stadium_id) t join
(select count(match_id) dd,stadium_id,stadium_name from ipl_match join ipl_match_schedule using(match_id) join ipl_stadium using(stadium_id) 
group by stadium_id) tt using(stadium_id) order by stadium_name;


-- 4.	Show the total bids along with bid team and team name.

select  bid_team, team_name , count(bid_team) as bid_counts from ipl_bidding_details a join ipl_team b on bid_team = team_id group by team_id;


-- 5.	Show the team id who won the match as per the win details.

select win_details, match_winner,if( match_winner=1 , team_id1 , team_id2) as team_id from ipl_match group by win_details;


-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.

select team_id,team_name, sum(matches_played), sum(matches_won), sum(matches_lost) from ipl_team join ipl_team_standings using(team_id) group by team_id;


-- 7.	Display the bowlers for Mumbai Indians team.

select player_id,player_name,player_role,a.remarks from ipl_team_players a join ipl_player b using(player_id) 
where team_id = (select team_id as id from ipl_team where team_name like "Mumbai%") and a.player_role = "Bowler";



-- 8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.

select team_id,team_name, count(player_id) as "count_of_allrounders" from ipl_team_players a join ipl_team b using(team_id) where player_role = "All-rounder" 
group by team_id having count_of_allrounders > 4;
